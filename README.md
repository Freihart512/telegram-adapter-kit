...
- **CI on PRs:** [`.github/workflows/pr-checks.yml`](.github/workflows/pr-checks.yml) runs `npm ci`, then **format:check**, **lint**, **typecheck**, **test**, **build**, and **verify:dist** on Node 20 (**TT-035**; satisfies the intent of **TT-030** until a separate doc-only follow-up is needed). Enable **branch protection** to require this check on `master` (**TT-038**).
